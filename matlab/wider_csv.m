...
% dataset_path = '../data/wider'
% split = 'train';
% output_file = ['../data/wider/WIDER_' split '/imglist.csv'];
...
for i=1:length(split_data.event_list)
    for j=1:length(split_data.file_list{i})
        ...
        fprintf(fhandle,'%s/%s.jpg,%d,%d,%d,%d\n',split_data.event_list{i},...
            ...
            round(split_data.face_bbx_list{i}{j}(k,2)),ceil(split_data.face_bbx_list{i}{j}(k,3)),...
            ceil(split_data.face_bbx_list{i}{j}(k,4)));
        end
    end
end
fclose(fhandle);
