...
if __name__ == "__main__":
    ...
    parser.add_argument('--ylimit', type=float, default=0,
                        help='Y limit when creating the pdf')
    ...
    pylab.legend(ii, ll, loc='lower left',prop={'size':16, 'weight':'bold'})
    pylab.xlabel("Recall",fontsize=18,fontweight='bold')
    pylab.ylabel("Precision",fontsize=18,fontweight='bold')
    pylab.tick_params(axis='both', which='major', labelsize=18)
    pylab.grid()
    pylab.gca().set_xlim((0, 1))
    pylab.gca().set_ylim((args.ylimit,1))
    savename = "%s_final.pdf" % args.dataset
    pylab.savefig(savename)
    savename = "%s_final.png" % args.dataset
    pylab.savefig(savename)
